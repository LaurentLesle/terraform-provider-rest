...
description: |-
  The rest provider provides resource and data source to interact with a platform that exposes a RESTful API.
---
...
- `arm_tenant_tokens` (Map of Strings, Sensitive) Map of tenant_id → ARM bearer token for cross-tenant access.
- `arm_token` (String, Sensitive) Bearer token for Azure Resource Manager. Used by validate_externals to verify azure_* external references.
...
- `fail_on_warning` (Boolean) When true, validate_externals raises an error if any API validation produces a warning. Defaults to false.
- `github_token` (String, Sensitive) GitHub token for the GitHub API. Used by validate_externals to verify github_* external references.
