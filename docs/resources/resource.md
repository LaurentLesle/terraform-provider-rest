...
page_title: "rest_resource Resource - terraform-provider-rest"
subcategory: ""
description: |-
  `rest_resource` manages a rest resource.
---
...
- `ephemeral_header` (Map of Strings, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) The ephemeral header parameters. This will be merged into the `header` for API calls but NOT persisted to state.
...
```shell
$ terraform import rest_resource.example "{
